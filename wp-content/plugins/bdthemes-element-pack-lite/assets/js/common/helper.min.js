...
function epObserveTarget(t, e) {
    ...
    s.rootMargin = s.rootMargin || "10% 0px 0px 0px", new IntersectionObserver((function(t, l) {
...
function EP_SAFE_HTML(t) {
    ...
    return t = t.replace(/<\/?([a-zA-Z0-9]+)([^>]*)>/g, ((t, s, l) => {
        ...
        const r = e[s];
        return `<${s}${l.replace(/([a-zA-Z0-9-]+)\s*=\s*("[^"]*"|'[^']*'|[^\s>]*)/g,((t,e,s)=>(e=e.toLowerCase(),!r.includes(e)||e.startsWith("on")||/^javascript:/i.test(s)?"":`${e}=${s}`)))}>`
    }))
}
...
jQuery, jQuery(document).ajaxComplete((function(t, e, s) {
